...
The right compiler version needs to either be in your $PATH, or it needs to be given as an explicit path to the script for this to work.  Mine is in the $PATH:
...
Assuming that a suitable test program exists (this can be a simple "Hello, World!" so long as it compiles easily), you are ready to go:
...
You can use the basic examples as a starting point for this.  The missing parts need to go at the top of the one that was just generated by the script.  For example:
